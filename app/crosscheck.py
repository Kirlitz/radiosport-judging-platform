...
def format_band(freq_str):
    ...
    try:
        ...
        if 144000 <= freq <= 146000 or 144 <= freq <= 146: return '2m'
        if 430000 <= freq <= 440000 or 430 <= freq <= 440: return '70cm'        
    except ValueError:
        pass
    return f"{freq_str}m"
    ...
    for cat_name in os.listdir(comp_dir):
        ...
        for filename in os.listdir(cat_path):
            if filename.endswith('.cbr'):
                # Имя файла: <позывной>_<id>.cbr — отбрасываем суффикс id
                callsign = re.sub(r'_\d+$', '', filename)[:-4].upper()
